...
int EmpCylSL::cache_grid(int readwrite, string cachefile)
{
  ...
  if (readwrite) {
	...
	for (int ix=0; ix<=NUMX; ix++)
	  for (int iy=0; iy<=NUMY; iy++)
	    in.read((char *)&zC[m][v][ix][iy], sizeof(double));
...
{
  //if (VFLAG & 4)
  std::cout << "Process " << setw(4) << myid << ": made it to pca_hall, hallfile=" << hallfile << " Compute=" << compute << std::endl;
  ...
  if (compute) {
    ...
    //if (VFLAG & 4)
    std::cout << "Process " << setw(4) << myid << ": mass " 
    ...
    if (myid==0)  std::cout << "0. Hallfile=" << hallfile;
    if (myid == MASTER)  std::cout << "0.MASTER Hallfile=" << hallfile;

    if (myid==1)  std::cout << "0.1 Hallfile=" << hallfile;
    if (myid==2)  std::cout << "0.2 Hallfile=" << hallfile;
    ...
    //if (hallfile.length()>0) {
    if (compute) {

      // diagnostic printing
      std::cout << "Process " << myid << " 1. Hallfile=" << hallfile;
      
      std::ostringstream ofile, mfile;
      ofile << hallfile << "." << myid << ".pcalog";

      // add .c_str() to opening as a tester
      hout.open(ofile.str().c_str(), ios::out | ios::app);
      if (hout.good()) {
	hout << "#" << endl << std::right
	     << "# Time = " << tnow << endl
	     << "#" << endl
	...
	std::cout << "Could not open <" << ofile.str() << "> for appending output" 
		  << std::endl;
      }
    
      mfile << hallfile << "." << myid << ".pcamat";

      // same string addition as above
      mout.open(mfile.str().c_str(), ios::out | ios::app);
      if (mout.good()) {
	mout << "#" << endl << std::right
	     << "# Time = " << tnow << endl
	     << "#" << endl << setprecision(4);
      } else {
	std::cout << "Could not open <" << mfile.str() << "> for appending output" 
		  << std::endl;
      }
    }
  ...
  //if (VFLAG & 4)
    std::cout << "Process " << setw(4) << myid << ": exiting to pca_hall" << std::endl;
}
