...
DiskHalo(SphericalSL* haloexp, SphericalSL* bulgeexp, EmpCylSL* diskexp,
	 ...
	 string& filename1, int DIVERGE, double DIVERGE_RFAC,   // real halo model
	 string& filename2, int DIVERGE2, double DIVERGE_RFAC2, // fake for multimass halo
	 string& filename3, int DIVERGE3, double DIVERGE_RFAC3, // bulge filename
	 DiskGenType type)
{
  disktableP = NULL;
  disktableN = NULL;
  ...
  if (VFLAG & 2) {
    halo4->print_model("diskhalo2_model.multi");
    halo4->print_df("diskhalo2_df.multi");
  }
  ...
  //
  multi = new SphericalModelMulti(halo3, halo4);
  multi -> gen_tolE = TOLE;
...
{
  newmoddisk = p.newmoddisk;
  newmodbulge = p.newmodbulge;
  halo   = p.halo;
  halo2  = p.halo2;
  halo3  = p.halo3;
  halo4  = p.halo4;
...
void DiskHalo::
set_bulge_coordinates(vector<Particle>& pbulge, int nbulge, int npart)
{
  const double tol = 1.0e-12;
  double rmin = max<double>(bulge->get_min_radius(), RHMIN);
  double rmax = bulge->get_max_radius();
  double mmin = bulge->get_mass(rmin);
  double mtot = bulge->get_mass(rmax);

  cout << setw(18) << "SET BULGE COORDINATES: MTOT=" << setw(18) << mtot << endl;
  ...
  if (VFLAG & 1)
    cout << "Process " << myid << ": made " << pbulge.size() << " particles"
	 << endl;
...
double DiskHalo::
get_bpot(double xp, double yp, double zp)
{
  if (!expandh) return 0.0;
  ...
  expandb->determine_fields_at_point(r, theta, phi,
				     &dens, &potl, &potr, &pott, &potp);
...
double DiskHalo::
get_bdpot(double xp, double yp, double zp)
{
  if (!expandb) return 0.0;
...
deri_pot(double xp, double yp, double zp, int n)
{
  ...
  switch(n){
  case 0:
    dP = deri_pot_disk(xp,yp,zp,0) + deri_pot_halo(xp,yp,zp,0) + deri_pot_bulge(xp,yp,zp,0);
    break;
  case 1:
    dP = deri_pot_disk(xp,yp,zp,1) + deri_pot_halo(xp,yp,zp,1) + deri_pot_bulge(xp,yp,zp,1);
    break;
  default:
...
double DiskHalo::
deri_pot_bulge(double xp, double yp, double zp, int n)
{
  double dP=0.0;
  
  switch(n) {
  case 0:
    dP = get_bpot(xp,yp,zp);
    break;
  case 1:
    dP = get_bdpot(xp,yp,zp);
    break;
  default:
    cout<<"deri_pot_bulge: derivative of order " << n 
	<< " can't be calculated" << endl;
    break;
  }
  return dP;
...
table_disk(vector<Particle>& part)
{
  ...
  double densb, potlb, dprb, dptb, dppb;

				// Add no force if no component exists
  pot = fr = fz = fp = dens = potl = dpr = dpt = dpp = densb = potlb = dprb = dptb = dppb =0.0;
  ...
  Matrix workV (0, 4, 0, NDR-1);
				...
				// For debugging
  Matrix workD (0, 10, 0, NDR-1);
  Matrix workDZ(0, 8, 0, NDZ-1);
  ...
  for (int i=ibeg[myid]; i<iend[myid]; i++) {
	...
	expandh->determine_fields_at_point(R, 0.5*M_PI, phi,
      ...
      if (expandb)
	expandb->determine_fields_at_point(R, 0.5*M_PI, phi,
					   &densb, &potlb, &dprb, &dptb, &dppb);
      ...
      workE[j]    = max<double>(-fr + dpr + dprb, 1.0e-20);
      ...
      if (i==0) {
	workD[4][j] = -fr;
	// change to have the bulge potential added to halo
	workD[5][j] = dpr;// + dprb;
	workD[6][j] = potl;
	workD[7][j] = dens;
	workD[8][j] = dprb;
	...
	dpdz = -fz + dpr*z/r + dpt*R*R/(r*r*r) + dprb*z/r + dptb*R*R/(r*r*r);

				// Debugging
	workDZ[0][k] = disk_density(R, z);
	workDZ[1][k] = -fz;
	workDZ[2][k] = dpr*z/r + dprb*z/r ;
	workDZ[3][k] = dpt*R*R/(r*r*r) + dptb*R*R/(r*r*r);
	...
	for (int k=0; k<NDZ; k++) {
	  dout << setw(15) << workZ[k] 
	       << setw(15) << workP[k] 
	       << setw(15) << workA[k];
	  for (int q=0; q<9; q++) dout << setw(15) << workDZ[q][k];
	  dout << endl;
	}
	dout.close();
      }
  ...
  if (myid==curid && expandh && VFLAG & 4) {
    ...
    for (int j=0; j<NDR; j++) {
      ...
      rho = halo->get_density(r) + bulge->get_density(r);

      deriv = (get_bulge_disp(0.0, r1, 0.0)*halo->get_density(r1) - 
	       get_bulge_disp(0.0, r2, 0.0)*halo->get_density(r2) ) /	(r1 - r2);
	...
	<< setw(14) << workD[8][j]            // #25  bulge force
	<< setw(14) << workD[9][j]            // #26  bulge potential (from fit)
	<< setw(14) << workD[10][j]            // #27  bulge density (from fit)
	  << endl;
    }
...
{
  // updated to include halo3 as the composite potential with bulge 09-04-2016
  if (!MULTI) {
  ...
  double Emax = halo3->get_pot(rmax);
  ...
  for (int i=ibeg; i<iend; i++) {
    ...
    for (int n=1; n<=nlq; n++) {
      ...
      if (v2<0.0) v2 = 0.0;
      v = sqrt(v2);
      fac = (Emax - pot)*lq.weight(n)*halo3->distf(E, 0.5);
				// velocity dispersion
...
table_bulge(vector<Particle>& part)
{
  ...
  double densb, potlb, dprb, dptb, dppb, dpdrb;
  ...
  for (int i=ibeg[myid]; i<iend[myid]; i++) {
    ...
    for (int j=0; j<NHR; j++) {
      ...
      // add bulge field
      expandb->determine_fields_at_point(r, theta, 0.0,
					 &densb, &potlb, &dprb, &dptb, &dppb);
      
      dpdr = dpr + dprb - fz*cos(theta) - fr*sin(theta);

      // 
      work[j] = bulge->get_density(r) * dpdr * r;
    ...
    for (int j=0; j<NHR; j++) {
      bulgetable[i][j] = max<double>(workA[NHR-1] - workA[j], MINDOUBLE);
      if (fabs(bulgetable[i][j])>1.0e8) {
	cerr << "Oops, val=" << bulgetable[i][j] << endl;
      }
    }
  }
  ...
  for (int k=0; k<numprocs; k++) {
    for (int i=ibeg[k]; i<iend[k]; i++) {
      MPI_Bcast(&bulgetable[i][0], NHR, MPI_DOUBLE, k, MPI_COMM_WORLD);
    }
  }
  ...
  if (myid==0 && expandh && VFLAG & 4) {
	...
	out << setw(18) << -1.0 + dc*j
	    ...
	    << setw(18) << bulgetable[j][k] << endl;
      }
      out << endl;
    }
    
    ostringstream sout2;
    sout2 << "test_bulge_pot." << RUNTAG;
    ...
    for (int k=0; k<NHR; k++) {
      ...
      expandh->determine_fields_at_point(rr, 0.5*M_PI, 0.0,
					 &t, &p, &dp, &t, &t);
      expandb->determine_fields_at_point(rr, 0.5*M_PI, 0.0,
					 &tb, &pb, &dpb, &tb, &tb);
      out2
	<< setw(14) << rr
	<< setw(14) << p + pb
	<< setw(14) << dp + dpb
	<< setw(14) << get_bulge_disp(rr, 0.0, 0.0)
	<< endl;
    }
	...
	zz = rr*costh;
	out3 
	  << setw(14) << bulgetable[j][k] -  
	  get_bulge_disp(xx, 0.0, zz) * bulge->get_density(rr);
      }
      out3 << endl;
    }
  }
...
double DiskHalo::get_bulge_disp(double xp,double yp, double zp)
{
  if (MULTI) {
    double r = sqrt(xp*xp + yp*yp + zp*zp);
    r = max<double>(r, halo3->get_min_radius());
    r = min<double>(r, halo3->get_max_radius());
    return odd2(r, bulgetable[0], bulgetable[1], 0);
  }

  if (bulgetable.getchigh() != NHR-1) {
    cerr << "DiskHalo::get_bulge_disp: must call table_bulge first\n";
    MPI_Abort(MPI_COMM_WORLD, 100);
    exit(0);
  }
  ...
  resv = 
    ...
    ct[0]*cr[0] * bulgetable[it  ][ir  ]  +
    ct[0]*cr[1] * bulgetable[it  ][ir+1]  +
    ct[1]*cr[0] * bulgetable[it+1][ir  ]  +
    ct[1]*cr[1] * bulgetable[it+1][ir+1]  ;
  
  double dens = bulge->get_density(r);
  if (dens>0.0) resv /= dens;
  
  return resv;
}

void DiskHalo::set_vel_bulge(vector<Particle>& part)
{
  // the passed halo must the the halo with correct mass distribution
  if (!expandb) {
    if (myid==0) cout << "[no bulge particles] ";
    return;
  }
  ...
  // see what needs to happen here for the bulge alone?
  table_bulge(part);
  ...
  for (auto &p : part) {
	...
	cout << "gen_velocity failed: "
	     ...
	     << p.pos[2] << "\n";
				...
				// Use Jeans
    if (nok) {
      v2r = get_bulge_disp(p.pos[0], p.pos[1], p.pos[2]);
...
void DiskHalo::write_file(ostream &fou_halo,  ostream &fou_disk, ostream &fou_bulge,
			  vector<Particle>& hpart, vector<Particle>& dpart, vector<Particle>& bpart)
{
  // hopefully rewritten for bulge 09-04-2016
  int l  = hpart.size();
  int l1 = dpart.size();
  int l2 = bpart.size();
  ...
  if (myid==0) {
    ...
    fou_bulge.setf(ios::scientific);
    
    fou_halo << nhalo << " " << 0 << " " << 0 << endl;
    fou_disk << ndisk << " " << 0 << " " << 0 << endl;
    fou_bulge << nbulge << " " << 0 << " " << 0 << endl;
    ...
    if (VFLAG & 1) {
      ...
      cout << "Bulge stream is ";
      if (fou_bulge.good()) cout << "GOOD\n";
      else cout << "BAD\n";
    }
    ...
    for (int i=0; i<l2; i++)
      write_record(fou_bulge, bpart[i]);
    
    if (VFLAG & 1) {
      cout << "Wrote " << l  << " HALO particles from Node 0" << endl;
      cout << "Wrote " << l1 << " DISK particles from Node 0" << endl;
      cout << "Wrote " << l2 << " BULGE particles from Node 0" << endl;
    }
	...
	for (int i=0; i<icur; i++) write_record(fou_bulge, buf[i]);
	ccnt += icur;
      }
      
      if (VFLAG & 1)
	cout << "Wrote " << ccnt << " BULGE particles from Node " << n << endl;
	...
	while (icur<l2) {
	  ...
	  for (int j=0; j<ipack; j++) buf[j][bpart[icur+j]];
	...
	if (VFLAG & 1)
	  cout << "Sent " << icur << " BULGE particles from Node " << n << endl;
