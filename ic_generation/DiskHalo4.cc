...
//
// Multimass case is the only version supported right now
//
DiskHalo::
DiskHalo(SphericalSL* haloexp, EmpCylSL* diskexp1, EmpCylSL* diskexp2,
	 double HH1, double AA1, double DMass1,
	 double HH2, double AA2, double DMass2,
	 ...
	 DiskGenType type)
{
  ...
  this->type = type;
    ...
    cout << "Process " << myid << ": "
	 ...
	 << " DMASS2=" << DMass2
      << " DIVERGE1=" << DIVERGE
      << " DIVERGE_RFAC1=" << DIVERGE_RFAC
      << " DIVERGE2=" << DIVERGE2
      << " DIVERGE_RFAC2=" << DIVERGE_RFAC2
	 << endl << flush;
    }
  ...
  halo1 = new SphericalModelTable(filename1, DIVERGE, DIVERGE_RFAC);

  if (myid==0) cout << "HALO MADE" << endl;

  disk1 = new ExponentialDisk(AA1, HH1, RDMAX, DMass1);

  //disk1->test_threedee();

  if (myid==0) cout << "DISK1 MADE" << endl;
  ...
  //
  // Add the second disk potential to the halo model
  //
  ...
  disk2 = new ExponentialDisk(AA2, HH2, RDMAX, DMass2);

  // This modifies the mass itself instead of just taking the actual disk mass. Might be a huge problem.
    newmod = new AddDisk(halo2, disk2, COMPRESSION); 
  halo3 = newmod->get_model();
  halo3->setup_df(NUMDF, RA);
  if (myid==0 && VFLAG & 2) {
    char debugname[] = "df3.debug";
    halo3->print_df(debugname);
    halo3->print_model("diskhalo4_model_halo3.multi");
  }


  if (myid==0) cout << "DF3 MADE" << endl;
  ...
  halo4 = new SphericalModelTable(filename2, DIVERGE2, DIVERGE_RFAC2);
  ...
  double *d3 = new double [RNUM]; // for the original values of density and mass
  double *m3 = new double [RNUM];
  
  double rmin = halo3->get_min_radius();
  double rmax = halo3->get_max_radius();
  ...
  for (int i=0; i<RNUM; i++) {
    if (LOGR)
      ...
      r2[i] = r = rmin + dr*i;
    d2[i] = halo4 -> get_density(r);
    m2[i] = halo4 -> get_mass(r);
    p2[i] = halo3 -> get_pot(r);
    d3[i] = halo1 -> get_density(r); // revert to original, which is what halo3 is carrying around
    m3[i] = halo1 -> get_mass(r);
  }
  
  delete halo4;

  // CREATE A NEW MODEL WITH ORIGINAL DENSITY, ORIGINAL MASS, DOUBLE DISK ADDED POTENTIAL
    halo5 = new SphericalModelTable(RNUM, r2-1, d3-1, m3-1, p2-1, 
				  DIVERGE2, DIVERGE_RFAC2);

          halo5->setup_df(NUMDF,RA);

      if (VFLAG & 2) {
    halo5->print_model("diskhalo4_model_halo5.multi");
  }


  halo4 = new SphericalModelTable(RNUM, r2-1, d2-1, m2-1, p2-1, 
				  DIVERGE2, DIVERGE_RFAC2);
  halo4->setup_df(NUMDF, RA);
  if (VFLAG & 2) {
    halo4->print_model("diskhalo4_model_halo4.multi");
    halo4->print_df("diskhalo4_df.multi");
  }
  ...
  //
  multi = new SphericalModelMulti(halo5, halo4);
  multi -> gen_tolE = TOLE;

  // For frequency computation
  //
  hDmin = log(max<double>(disk1->get_min_radius(), RDMIN));
  hDmax = log(disk1->get_max_radius());
  dRh   = (hDmax - hDmin)/nh;
...
DiskHalo::DiskHalo(const DiskHalo &p)
{
  ...
  scaleheight1 = p.scaleheight1;
  scaleheight2 = p.scaleheight2;
  dmass1 = p.dmass1;
  dmass2 = p.dmass2;
...
double DiskHalo::double_disk_density(ExponentialDisk* disk1,
				     ExponentialDisk* disk2,
				     double R, double z)
{
  return disk1->disk_density( R, z) + disk2->disk_density(R,z);
}

double DiskHalo::double_disk_surface_density(ExponentialDisk* disk1, ExponentialDisk* disk2,
					     double R)
{
  return disk1->get_density(R) + disk2->get_density(R);
}
...
void DiskHalo::set_halo(vector<Particle>& phalo, int nhalo, int npart)
{
  ...
  // this used to be halo1. seems like that is correct, this should be the actual halo mass. potential is another concern...
  double rmin = max<double>(halo1->get_min_radius(), RHMIN);
  double rmax = halo1->get_max_radius();
  double mmin = halo1->get_mass(rmin);
  double mtot = halo1->get_mass(rmax);
...
void DiskHalo::
set_disk_coordinates(ExponentialDisk* disk, vector<Particle>& pdisk, int ndisk, int npart)
{
  // can this be changed to take a disk instantiation, or do I need a completely separate call for the dark disk?
  ...
  if (myid==0) cout << "DISK1 PARTICLE MASS IS "<< mtot/ndisk << endl;
  ...
  for (int i=0; i<npart; i++) {
    ...
    p.pos[0] = R*cos(phi);
    p.pos[1] = R*sin(phi);
    p.pos[2] = disk->get_hscale()*atanh(2.0*(*rndU)()-1.0);
...
// TODO
// Generalize all functions to take different expansions and different masses
void DiskHalo::disk_eval(EmpCylSL* expandd, double dmass, double R, double z, double phi,
...
{
  if (!expandd1) return 0.0;
  if (!expandd2) return 0.0;

  double r, phi=0.0, pot1=0.0, pot2=0.0;
  double fr1=0.0, fz1=0.0, fp1=0.0, fr2=0.0, fz2=0.0, fp2=0.0;
  
  r = sqrt(xp*xp + yp*yp);
  disk_eval(expandd1, dmass1, r, zp, phi, pot1, fr1, fz1, fp1);
  disk_eval(expandd2, dmass2, r, zp, phi, pot2, fr2, fz2, fp2);
    
  return pot1+pot2;
}

// these have both been converted to double versions
double DiskHalo::
...
get_ddpot(double xp, double yp, double zp)
{
  ...
  double pot1=0.0, pot2=0.0, fr1=0.0, fz1=0.0, fp1=0.0, fr2=0.0, fz2=0.0, fp2=0.0;
...
table_double_disk(vector<Particle>& part1, vector<Particle>& part2)
{
  ...
  for (auto &p : part1) {
    ...
    double maxscl = max<double>(disk1->get_hscale()*SHFACTOR, disk2->get_hscale()*SHFACTOR);
    maxz1 = max<double>(maxscl,maxz1);
  ...
  Vector workVfourlog(0, NDR-1);
  Vector workQ2smooth(0, NDR-1);
  Vector workVfoursmooth(0, NDR-1);
  ...
  for (int i=ibeg[myid]; i<iend[myid]; i++) {
    ...
    for (int j=0; j<NDR; j++) {
      ...
      disk_eval(expandd2, dmass2, R, 0.0, phi, pot2, fr2, fz2, fp2);
	...
	expandh->determine_fields_at_point(R, 0.5*M_PI, phi,
      ...
      workE[j]    = max<double>(-fr1 - fr2 + dpr, 1.0e-20);
	...
	r = sqrt(R*R + z*z) + MINDOUBLE;
				...
				// Positive
	disk_eval(expandd1, dmass1, R, z, phi, pot1, fr1, fz1, fp1);
	disk_eval(expandd2, dmass2, R, z, phi, pot2, fr2, fz2, fp2);
    ...
    */
	...
	//asytable[i][j] = drv2(workV[0][j], workV[0], workVfoursmooth, 1);
      else
  ...
  if (myid==curid && expandh && VFLAG & 4) {
    ...
    for (int j=0; j<NDR; j++) {
      ...
      // I don't think these actually matter for the determination, so halo1 should be a safe choice
      rho = halo1->get_density(r);

      deriv = (get_disp(0.0, r1, 0.0)*halo1->get_density(r1) - 
	       get_disp(0.0, r2, 0.0)*halo1->get_density(r2) ) /	(r1 - r2);
      
      lhs = halo1->get_mass(r);
      rhs = -r*r*deriv/rho;

      if (CHEBY)
	// test case for log version of CHEBY
	deriv2 = drv2(workR[j], workV[0], workQ2smooth);
	  ...
	  << setw(14) << rho			// #16
	//<< setw(14) << deriv			// #17
	  << setw(14) << a_drift(r, 0.0, 0.0)	// #17 // curiousity check
	// << setw(14) << lhs			// #18
	//  << setw(14) << rhs			// #19
	  << setw(14) << disk1->get_density(r)			// #18
	  << setw(14) << disk2->get_density(r)			// #19
	  << setw(14) << lhs - rhs		// #20
  ...
  if (myid==curid && VFLAG & 4) {
	...
	out << setw(18) << RDMIN*exp(dR*j)
	    ...
	    << setw(18) << double_disk_density(disk1, disk2, RDMIN*exp(dR*j), dZ*k)
	    << endl;
      }
      out << endl;
    }
...
double DiskHalo::vz_disp2(double xp,double yp, double zp)
{
  ...
  // converted to double: NOT SURE ABOUT NEEDING THE MASS IN HERE
  double dens = double_disk_density(disk1, disk2, R, zp);
...
{
  // converted to take double disk
  double r = sqrt(xp*xp + yp*yp);
  if (r > 10.0*max(scalelength1,scalelength2)) return 0.0;
  double sigmar = 3.36*double_disk_surface_density(disk1, disk2, r)*Q/epi(xp, yp, zp);
...
void DiskHalo::
set_vel_double_disk(vector<Particle>& part1,vector<Particle>& part2)
{
  if (!expandd1 || !expandd2) {
    if (myid==0) cout << "[no disk particles for one of the components] ";
    return;
  }
  ...
  // EITHER COPY-PASTE OR MAKE A STANDALONE FUNCTION...

  //====================================ASSIGN PARTICLE VELCITIES=========================
  for (auto &p : part1) {
    ...
    //switch (type) { // SWITCH HAS BEEN DISABLED...GO BACK TO DISKHALO2 TO SEE THE WORKING VERSION
    //case Asymmetric:
    ...
    massp1 += p.mass;
    for (int k=0; k<3; k++) vel1[k] += p.mass*p.vel[k];
  } // end assignments of particle properties

  
  //====================================ASSIGN PARTICLE 2 VELCITIES=========================
  for (auto &p : part2) {
				// From solution to Jeans' equations in
				// cylindrical coordinates
    x = p.pos[0];
    ...
    massp1 += p.mass;
    for (int k=0; k<3; k++) vel1[k] += p.mass*p.vel[k];
  } // end assignments of particle properties (2)

  //=======================================GET LIMIT PARTICLE PROPERTIES===================================
...
void DiskHalo::table_halo_disp()
{
  ...
  if (myid==0 && VFLAG & 4) {
    ...
    for (int k=0; k<NHR; k++)
	  ...
	  << setw(18) << halo1->get_density(halotable[0][k]) << endl;
  }
...
table_halo(vector<Particle>& part)
{
  ...
  for (int i=ibeg[myid]; i<iend[myid]; i++) {
    ...
    for (int j=0; j<NHR; j++) {
      ...
      // these needs to be double disked
      if (expandd1) disk_eval(expandd1, dmass1, x, z, 0.0, pot, fr, fz, fp);
      ...
      work[j] = halo1->get_density(r) * dpdr * r;
  ...
  if (myid==0 && expandh && VFLAG & 4) {
	...
	out3 
	  ...
	  get_disp(xx, 0.0, zz) * halo1->get_density(rr);
      }
      out3 << endl;
    }
  }
...
{
  if (MULTI) {
    //why was this halo2?? pushes down further for the disk addition, I suppose?
    double r = sqrt(xp*xp + yp*yp + zp*zp);
    r = max<double>(r, halo3->get_min_radius());
    r = min<double>(r, halo3->get_max_radius());
  ...
  double dens = halo1->get_density(r);
...
void DiskHalo::write_record(ostream &out, SParticle &p)
{
 ...
 void DiskHalo::write_double_disk_file(ostream &fou_halo,  ostream &fou_disk1, ostream &fou_disk2,
				      vector<Particle>& hpart, vector<Particle>& dpart1, vector<Particle>& dpart2)
{
  int l  = hpart.size();
  int l1 = dpart1.size();
  int l2 = dpart2.size();
  ...
  // Get particle totals
  //
  int ndisk1=0, ndisk2=0, nhalo=0;
  MPI_Reduce(&l,  &nhalo, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&l1, &ndisk1, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&l2, &ndisk2, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
  ...
  if (myid==0) {
    ...
    if (VFLAG & 1)
      cout << endl
	   ...
	   << " ndisk1=" << ndisk1 << " ndisk2=" << ndisk2 << endl;
    
    fou_halo.setf(ios::scientific);
    fou_disk1.setf(ios::scientific);
    fou_disk2.setf(ios::scientific);
    
    fou_halo << nhalo << " " << 0 << " " << 0 << endl;
    fou_disk1 << ndisk1 << " " << 0 << " " << 0 << endl;
    fou_disk2 << ndisk2 << " " << 0 << " " << 0 << endl;
    ...
    if (VFLAG & 1) {
      ...
      cout << "Disk I stream is ";
      if (fou_disk1.good()) cout << "GOOD\n";
      else cout << "BAD\n";

      cout << "Disk II stream is ";
      if (fou_disk2.good()) cout << "GOOD\n";
      else cout << "BAD\n";
    }
    ...
    for (int i=0; i<l1; i++)
      write_record(fou_disk1, dpart1[i]);

    for (int i=0; i<l2; i++)
      write_record(fou_disk2, dpart2[i]);
    
    if (VFLAG & 1) {
      cout << "Wrote " << l  << " HALO particles from Node 0" << endl;
      cout << "Wrote " << l1 << " DISK I particles fron Node 0" << endl;
      cout << "Wrote " << l2 << " DISK II particles fron Node 0" << endl;
    }
	...
	for (int i=0; i<icur; i++) write_record(fou_disk1, buf[i]);
	ccnt += icur;
      }
      
      if (VFLAG & 1)
	cout << "Wrote " << ccnt << " DISK I particles from Node " << n << endl;

      // Disk II
            MPI_Recv(&imany, 1, MPI_INT, n, 16, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      ccnt = 0;
      while (ccnt<imany) {
	MPI_Recv(&icur, 1, MPI_INT, n, 17, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	MPI_Recv(&buf[0], icur, spt(), n, 18, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	for (int i=0; i<icur; i++) write_record(fou_disk2, buf[i]);
	ccnt += icur;
      }
      
      if (VFLAG & 1)
	cout << "Wrote " << ccnt << " DISK II particles from Node " << n << endl;
	...
	while (icur<l1) {
	  ...
	  for (int j=0; j<ipack; j++) buf[j][dpart1[icur+j]];
	...
	if (VFLAG & 1)
	  cout << "Sent " << icur << " DISK I particles from Node " << n << endl;

	// DISK 2	
	MPI_Send(&l2, 1, MPI_INT, 0, 16, MPI_COMM_WORLD);
	icur = 0;
	while (icur<l2) {
	  ipack = min<int>(l2-icur, NBUF);
	  for (int j=0; j<ipack; j++) buf[j][dpart2[icur+j]];
	  MPI_Send(&ipack, 1, MPI_INT, 0, 17, MPI_COMM_WORLD);
	  MPI_Send(&buf[0], ipack, spt(), 0, 18, MPI_COMM_WORLD);
	  icur += ipack;
	}

	if (VFLAG & 1)
	  cout << "Sent " << icur << " DISK II particles from Node " << n << endl;
...
void DiskHalo::virial_ratio(vector<Particle>& hpart, vector<Particle>& dpart)
{
  ...
  for (auto p : hpart) {
    ...
    if (expandd1) 
      disk_eval(expandd1, dmass1, R, zz, phi, potl, fr, fz, fp);
