...
double Halo::RHMIN       = 1.0e-4;
double Halo::RHMAX       = 50.0;
	 ...
	 DiskGenType type)
{
  ...
  for (int i=0; i<RNUM; i++) {
    ...
    d3[i] = halo1 -> get_density(r); // revert to original, which is what halo2 is carrying around
  ...
  if (VFLAG & 2) {
    ...
    halo3->print_df("diskhalo4_df.multi");
  }
  ...
  //
  multi = new SphericalModelMulti(halo4, halo3);
  multi -> gen_tolE = TOLE;
...
double DiskHalo::disk_density(ExponentialDisk* disk1,
				     double R, double z)
{
  return disk1->disk_density( R, z);
}

double DiskHalo::disk_surface_density(ExponentialDisk* disk1,
					     double R)
{
  return disk1->get_density(R);
}
...
void DiskHalo::
table_disk(vector<Particle>& part1)
{
  ...
  for (auto &p : part1) {
    ...
    double maxscl = disk1->get_hscale()*SHFACTOR;
  ...
  for (int i=ibeg[myid]; i<iend[myid]; i++) {
	...
	workN[k] = disk_density(disk1, R, z) * dpdz;
      }
  ...
  if (myid==curid && expandh && VFLAG & 4) {
	...
	deriv2 = drv2(workR[j], workV[0], workQ2smooth);
      ...
      vrq0 = 3.36*(disk1->get_density(r))*Q/epi(r, 0.0, 0.0);
      vrq1 = 3.36*(disk1->get_density(r))*Q/sqrt(epitable[0][j]);
	...
	//  << setw(14) << rhs			// #19
	  ...
	  << setw(14) << disk1->get_density(r)			// #19
	  << setw(14) << lhs - rhs		// #20
  ...
  if (myid==curid && VFLAG & 4) {
	...
	out << setw(18) << RDMIN*exp(dR*j)
	    ...
	    << setw(18) << disk_density(disk1,RDMIN*exp(dR*j), dZ*k)
	    << endl;
      }
      out << endl;
    }
...
double DiskHalo::vz_disp2(double xp,double yp, double zp)
{
  ...
  double dens = disk_density(disk1, R, zp);
...
double DiskHalo::vr_disp2(double xp, double yp,double zp)
{
  ...
  double r = sqrt(xp*xp + yp*yp);
  if (r > 10.0*scalelength1) return 0.0;
  double sigmar = 3.36*disk_surface_density(disk1, r)*Q/epi(xp, yp, zp);
...
void DiskHalo::
set_vel_disk(vector<Particle>& part1)
{
  if (!expandd1) {
...
void DiskHalo::write_record(ostream &out, SParticle &p)
{
 ...
 void DiskHalo::write_disk_file(ostream &fou_halo,  ostream &fou_disk1,
  ...
  if (myid==0) {
    ...
    if (VFLAG & 1)
      cout << endl
	   ...
	   << " ndisk1=" << ndisk1 << " ndisk1=" << ndisk1 << endl;
    ...
    if (VFLAG & 1) {
      cout << "Wrote " << l  << " HALO I particles from Node 0" << endl;
	...
	if (VFLAG & 1)
	  cout << "Sent " << icur << " HALO I particles from Node " << n << endl;
